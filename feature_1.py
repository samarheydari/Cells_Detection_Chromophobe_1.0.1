...
def step1(read_type):
    ...
    #os_path = "C:\\Users\\samar\\Downloads\\Cells_Detection_Chromophobe_1.0.1-main\\Cells_Detection_Chromophobe_1.0.1-main\\"
    cell_area_hist_list=[]
    print("============Step 1 Start============")
    #-----read-----
   ...
   # else:
        #file_path="C:\\Users\\samar\\Downloads\\Cells_Detection_Chromophobe_1.0.1-main\\Cells_Detection_Chromophobe_1.0.1-main\\bin\\output\\"+str(read_type)+".jpg"
    ...
    if not os.path.exists(os_path+'\\result'):
        os.makedirs(os_path + '\\result')
    if not os.path.exists(os_path+'\\output_single'):
        os.mkdir(os_path + '\\output_single')
    if not os.path.exists(os_path+'\\result_pdf'):
    ...
    #cv.imshow("gray", gray)
    cv.imwrite("bin\\black-and-white.jpg",gray)

    gauss = cv.GaussianBlur(gray, (5, 5), 5)
    cv.imwrite("bin\\gauss1.jpg",gauss)
    ...
    print("============Step 1 End============")
    cv.waitKey()
    return counter_number,Cells_density,ave_cell_nucleus_color,threshold_value
